...
from app.common.response import MetaInfo, success_response
...
def create_v1_router(facade: Any) -> APIRouter:
    ...
    def get_results(batch_id: str) -> dict[str, Any]:
        ...
        return success_response(data=entry).model_dump()
    ...
    def get_request_result(batch_id: str, seq: int) -> dict[str, Any]:
        ...
        if entry is None:
            raise NotFoundError(f"Request seq {seq} not found in batch {batch_id}")
        return success_response(data=entry).model_dump()
    ...
    def list_batches(limit: int = Query(default=50, ge=1, le=500)) -> dict[str, Any]:
        batches = facade.list_batches(limit=limit)
        meta = MetaInfo(page=1, per_page=limit, total=len(batches))
        return success_response(data=batches, meta=meta).model_dump()

    @router.get("/admin/cleanup")
    def cleanup(days: int = Query(default=7, ge=1, le=365)) -> dict[str, Any]:
        result = facade.cleanup(days=days)
